 ...
 */
    ...
    public void sendSms(String phoneNumber) {
        ...
        if (ressources.isEmpty()) {
            System.out.println("⚠️ Aucune ressource disponible pour générer un rapport.");
            return;
        }

        // 🔹 Récupération des valeurs Twilio depuis TwilioConfig



        StringBuilder contenu = new StringBuilder("📊 Demandes des Ressources 📊\n\n");
        for (Ressource res : ressources) {
            String etat = res.getNombreRessource() == 0 ? "Épuisée ❌" :
                    res.getNombreRessource() <= 50 ? "Critique ⚠️" : "Disponible ✅";
            contenu.append("🔹 ").append(res.getNomRessource())
                    .append(" : ").append(res.getNombreRessource()).append(" unités (")
                    .append(etat).append(")\n");
        }
        ...
        try {
            System.out.println("🛠️ Initialisation Twilio...");
            Twilio.init("AC0a7cc7f5ecb13f10c10c1b00f537474e", "3333fda5234530e796f8f4889f1c71c5");

            System.out.println("📩 Envoi du SMS en cours...");
            Message message = Message.creator(
                    new PhoneNumber(phoneNumber),
                    new PhoneNumber("+19064839730"),
                    contenu.toString()
            ).create();
...
*/
    ...
    public void sendSms2(String to) {
        ...
        // Filtrer uniquement les ressources ayant un nombre < 50
        List<Ressource> lowStockRessources = ressources.stream()
                .filter(r -> r.getNombreRessource() < 100)
                .collect(Collectors.toList());

        if (lowStockRessources.isEmpty()) {
            System.out.println("✅ Aucune ressource en stock faible. Pas d'envoi de SMS.");
            return;
        }

        // Construire le message
        String messageBody = lowStockRessources.stream()
                .map(r -> r.getNomRessource() + " : " + r.getNombreRessource())
