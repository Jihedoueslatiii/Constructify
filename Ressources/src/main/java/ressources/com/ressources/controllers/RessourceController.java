...
public ResponseEntity<Map<Integer, Integer>> getProjectsForRessource(@PathVariable Long id) {
    ...
    @GetMapping("/sendSms1")
    public ResponseEntity<String> envoyerSms1() {

        ressourceService.sendSms1("+21699168074","ff");
        return ResponseEntity.ok("📩 SMS envoyé avec succès à " );
    }

    @GetMapping("/sendSms2/{numero}")
    public ResponseEntity<String> envoyerSms24(@PathVariable String numero) {
        ressourceService.sendSms2(numero);
